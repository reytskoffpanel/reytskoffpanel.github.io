class App {
    ...
    attachEventListeners() {
        ...
        document.getElementById('moderatorForm')?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await this.createWorker(workerData);
    ...
    async createWorker(workerData) {
        const { data: worker, error } = await this.supabaseService.createWorker(
            workerData.email,
            workerData.password,
            workerData.fullName,
    ...
    async loadWorkers() {
        try {
            ...
            // Сохраняем карточки создания новых пользователей
            const createCards = container.querySelectorAll('.worker-card');
            const createCardsHTML = Array.from(createCards)
                .filter(card => card.querySelector('.open-worker-modal'))
                .map(card => card.outerHTML)
                .join('');
            ...
            // Объединяем карточки создания и существующих пользователей
            container.innerHTML = createCardsHTML + workersHTML;
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadWorkers() {
        ...
        try {
            const workers = await supabase.getWorkers();
            const workersList = document.getElementById('workersList');
            
            if (workers.length === 0) {
                workersList.innerHTML = '<div class="no-workers">Нет созданных работников</div>';
                return;
            }
            ...
            const workersList = document.getElementById('workersList');
            workersList.innerHTML = '<div class="error">Ошибка загрузки работников</div>';
        }
    }
