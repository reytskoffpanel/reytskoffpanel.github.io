// Создаем единый экземпляр Supabase
...
class App {
    ...
    attachEventListeners() {
        ...
        document.getElementById('sellerForm')?.addEventListener('submit', async (e) => {
            console.log('Seller form submit event triggered');
            e.preventDefault();
            ...
            try {
                ...
                console.log('Submitting seller form with data:', workerData);
                
                const result = await this.supabaseService.createWorker(
                    workerData.email,
                    workerData.password,
                    workerData.fullName,
                    workerData.phone,
                ...
                console.log('Worker creation result:', result);
                ...
                let errorMessage = 'Ошибка при создании продавца: ';
                
                if (error.message.includes('User already registered')) {
                    errorMessage += 'Пользователь с таким email уже зарегистрирован';
                } else if (error.message.includes('duplicate key value')) {
                    errorMessage += 'Работник с таким email уже существует';
                } else if (error.message.includes('auth')) {
                    errorMessage += 'Ошибка при создании учетной записи: ' + error.message;
                } else {
        ...
        document.getElementById('deliveryForm')?.addEventListener('submit', async (e) => {
            console.log('Delivery form submit event triggered');
            e.preventDefault();
            
            // Проверяем, что все поля заполнены
            const email = document.getElementById('deliveryEmail').value;
            const password = document.getElementById('deliveryPassword').value;
            const fullName = document.getElementById('deliveryFullName').value;
            const phone = document.getElementById('deliveryPhone').value;
            const vehicleType = document.getElementById('deliveryVehicle').value;
            const deliveryArea = document.getElementById('deliveryArea').value;

            if (!email || !password || !fullName || !phone || !vehicleType || !deliveryArea) {
                alert('Пожалуйста, заполните все поля формы');
                return;
            }
            ...
            try {
                ...
                console.log('Submitting delivery form with data:', workerData);
                ...
                let errorMessage = 'Ошибка при создании доставщика: ';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadOrders() {
        ...
        try {
            const { data: orders, error } = await supabaseService.getOrders();
            if (error) throw error;
    ...
    async function showOrderDetails(orderId) {
        ...
        try {
            const { data: order, error } = await supabaseService.getOrderDetails(orderId);
            if (error) throw error;
            ...
            if (workers && workers.length > 0) {
                ...
                document.querySelectorAll('.delete-worker').forEach(button => {
                    button.addEventListener('click', async (e) => {
                        ...
                        if (confirm('Вы уверены, что хотите удалить этого работника?')) {
                            try {
                                await supabaseService.deleteWorker(workerId);
                                await loadWorkers();
                            } catch (error) {
                                console.error('Error deleting worker:', error);
                                alert('Ошибка при удалении работника: ' + error.message);
    ...
    async function loadProducts() {
        ...
        try {
            const { data: products, error } = await supabaseService.getProducts();
            if (error) throw error;
    ...
    async function createProduct(e) {
        ...
        try {
            let imageUrl = null;
            if (imageFile) {
                imageUrl = await supabaseService.uploadProductImage(imageFile);
            }

            const { error } = await supabaseService.createProduct({
                name,
                description,
                price,
                image_url: imageUrl
    ...
    async function editProduct(productId) {
        try {
            const { data: product, error } = await supabaseService.client
                .from('products')
                .select('*')
                .eq('id', productId)
                .single();
    ...
    async function deleteProduct(productId) {
        ...
        try {
            const { error } = await supabaseService.deleteProduct(productId);
            if (error) throw error;
            await loadProducts();
        } catch (error) {
    ...
    document.getElementById('moderatorForm')?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            await supabaseService.createWorker(
                workerData.email,
                workerData.password,
                workerData.fullName,
                workerData.phone,
